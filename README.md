...
- **`dsl`** — The LangSpec DSL and its compiler. The meta-language used to describe language specs (lexer rules, parser grammar, validation stages). Source files (`.lspec`) are compiled via `LangSpecCompilerCompile`, which returns a structured `LangSpecCompileResult` (LST, trace, syntax errors, validation entries). Diagnostic output (syntax errors, trace, LST dump) is optional and directed through `LangSpecDiagnosticSink` (e.g. `DefaultLangSpecDiagnosticSink()` for stdout). The dsl package does not depend on editor or sublime.
